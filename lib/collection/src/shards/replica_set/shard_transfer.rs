...
impl ShardReplicaSet {
    ...
    ///
    /// A `from_version` may be provided to start queueing the WAL from that specific version. If
    /// `None` is provided, it'll queue from the latest available WAL version at this time.
    ///
